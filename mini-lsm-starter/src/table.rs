...
impl BlockMeta {
    ...
    pub fn encode_block_meta(block_meta: &[BlockMeta], buf: &mut Vec<u8>) {
        ...
        // Reserve the space to improve performance, especially when the size of imcoming data is large.
        // 再申请一大部分空间，避免多次申请空间
        ...
        for meta in block_meta {
            ...
            buf.put_slice(meta.first_key.as_key_slice().raw_ref());
            buf.put_u16(meta.last_key.len() as u16);
            buf.put_slice(meta.last_key.as_key_slice().raw_ref());
        }
...
impl SsTable {
    ...
    pub fn open(id: usize, block_cache: Option<Arc<BlockCache>>, file: FileObject) -> Result<Self> {
        let len = file.size();
        // TODO: 理解：偏移量是4，是因为元数据的偏移量被存储在了文件的后4个字节中
        ...
        let bloom_filter = Bloom::decode(&raw_bloom).unwrap();
