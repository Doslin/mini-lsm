...
fn test_task4_integration() {
    ...
    let storage: Arc<LsmStorageInner> = Arc::new(
        LsmStorageInner::open(dir.path(), LsmStorageOptions::default_for_week1_test()).unwrap(),
    );
    storage.put(b"1", b"233").unwrap();
    storage.put(b"2", b"2333").unwrap();
    storage.put(b"3", b"2332212123").unwrap();
    storage.put(b"3", b"23333").unwrap();
    storage
        .force_freeze_memtable(&storage.state_lock.lock())
        .unwrap();
    // assertion `left == right` failed: expected value: b"3233333", actual value: b"23333"
    //   left: b"3233333" 这个说明
    storage.delete(b"1").unwrap();
    storage.delete(b"2").unwrap();
    storage.put(b"3", b"2333").unwrap();
    storage.put(b"4", b"423333").unwrap();
    storage
        .force_freeze_memtable(&storage.state_lock.lock())
        .unwrap();
    storage.put(b"1", b"1233333").unwrap();
    storage.put(b"3", b"3233333").unwrap();
    storage
        .force_freeze_memtable(&storage.state_lock.lock())
        .unwrap();

    {
        ...
        check_lsm_iter_result_by_key(
            &mut iter,
            vec![
                (Bytes::from_static(b"1"), Bytes::from_static(b"1233333")),
                (Bytes::from_static(b"3"), Bytes::from_static(b"3233333")),
                (Bytes::from_static(b"4"), Bytes::from_static(b"423333")),
            ],
        );
        assert!(!iter.is_valid());
        iter.next().unwrap();
    ...
    println!("exe second rules to filter");
    {
        ...
        check_lsm_iter_result_by_key(
            &mut iter,
            vec![(Bytes::from_static(b"3"), Bytes::from_static(b"3233333"))],
        );
        assert!(!iter.is_valid());
        iter.next().unwrap();
        iter.next().unwrap();
