...
impl LsmStorageInner {
    ...
    fn compact(&self, task: &CompactionTask) -> Result<Vec<Arc<SsTable>>> {
        ...
        match task {
            ...
            CompactionTask::Simple(SimpleLeveledCompactionTask {
                ...
                // TODO: support L0 compaction off 这里报错了,自己的水平还是不行啊
                Some(_) => {
    ...
    fn trigger_compaction(&self) -> Result<()> {
        ...
        let ssts_to_remove = {
            let _state_lock = self.state_lock.lock();
            println!("apply_compaction_result once");

            let (mut snapshot, files_to_remove) = self
                .compaction_controller
                .apply_compaction_result(&self.state.read(), &task, &output);
            // TODO 上面有错
            // 执着于 apple watch .watchface 的文件自己平常多备份, 自己的手表如果回到了之前的状态, 能在表面上唤起 shortcuts, 对我当前又有多大的裨益呢?
            // 其实是不太应该去做的,今天在这里至少搞了 1h。说话其实也挺快的. 为了用得也不太多的功能, 反而把自己搞累了, 时间安排不合理
            println!("apply_compaction_result once ok");
    ...
    fn trigger_flush(&self) -> Result<()> {
        ...
        if res {
            println!("trigger_flush next force_flush_next_imm_memtable");
